...
export const batteryProfiles = mysqlTable("battery_profiles", {
  id:                varchar("id", { length: 36 }).primaryKey(),
  name:              varchar("name", { length: 255 }).notNull(),
  chemistry:         mysqlEnum("chemistry", [
                       "alkaline", "lithium_primary", "liion", "nimh", "lead_acid", "other"
                     ]).notNull(),
...
export const maintenanceTemplates = mysqlTable("maintenance_templates", {
  ...
  taskType:        mysqlEnum("task_type", ["inspect", "clean", "lubricate", "test", "full_service", "recharge", "replace"])
