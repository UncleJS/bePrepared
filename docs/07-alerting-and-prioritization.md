...
| Severity | Trigger | UI Colour | Dashboard Priority |
|----------|---------|-----------|-------------------|
| `upcoming` | Within `alert_upcoming_days` days of due | Blue | Plan ahead |
| `due` | On or after due date, within grace window | Amber | Act now |
| `overdue` | Past due date + grace window | Red | Resolve immediately |
...
| `task_due` | `task_progress.next_due_at` (recurring) | "Monthly fuel level check overdue" |
| `policy` | Policy override removal / default change | "Water target reset to system default" |
...
flowchart TD
  ...
  W --> Q2[Query replacement\ndue lots]
  W --> Q3[Query maintenance\nschedules due]
  W --> Q4[Query overdue\nrecurring tasks]
  ...
  P --> SEV{Determine\nseverity}
  SEV -- today/past → grace --> DUE[severity: due]
  SEV -- past grace --> OVR[severity: overdue]
...
|-------|----------|----------------|-------|
| Medications expired 2 days ago | overdue | medical 5x | 5000 |
| Water filter maintenance overdue 1 day | overdue | water 4x | 4000 |
| Generator service due tomorrow | due | power 3x | 300 |
| Food rotation upcoming in 7 days | upcoming | food 2x | 20 |
...
stateDiagram-v2
  ...
  Resolved   --> [*]       : Archived after 30 days
  Active     --> Archived  : User dismisses without resolving
```
...
| Field | Description |
|-------|-------------|
| `is_read` | User has seen the alert |
| `is_resolved` | Action taken; condition cleared |
...
The worker uses an **upsert-by-entity** approach:

```
For each (household_id, entity_type, entity_id):
  IF active unresolved alert exists:
    IF new severity > existing severity → UPDATE severity
...
Step 3: For each alert, take the minimum required action:
  ...
  → Task due:    Complete task + record evidence
