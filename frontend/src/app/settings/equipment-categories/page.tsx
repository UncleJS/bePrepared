...
export default function EquipmentCategoriesPage() {
  ...
  const inUseCount = useMemo(
    () => (archiveId ? items.filter((i) => i.categoryId === archiveId).length : 0),
    [items, archiveId]
  );
  const candidates = useMemo(
    () => categories.filter((c) => c.id !== archiveId),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 rounded-lg border border-border bg-card p-4">
        ...
        <button
          type="button"
          className="rounded-md bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground inline-flex items-center justify-center gap-2 self-end"
          onClick={async () => {
            try {
                ...
                <td className="px-4 py-2.5 text-right">
                  {!c.isSystem && (
                    <div className="inline-flex gap-2">
                      ...
                      <button
                        type="button"
                        className="rounded-md border border-border px-2 py-1 text-xs text-destructive inline-flex items-center gap-1"
                        onClick={() => {
                          setArchiveId(c.id);
                          setReplacementId("");
                        }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 rounded-lg border border-border bg-card p-4">
          ...
          <div className="flex gap-2 self-end">
            <button
              ...
              onClick={async () => {
                try {
                  await apiFetch(`/equipment/${householdId}/categories/${editingId}`, {
                    method: "PATCH",
                    body: JSON.stringify({
                      name: editName,
                      slug: editSlug,
        ...
        <div className="rounded-md border border-yellow-700/50 bg-yellow-950/20 p-3 space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            ...
            <button
              ...
              className="rounded-md bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50 self-end"
              onClick={async () => {
                try {
                  const path = replacementId
                    ? `/equipment/${householdId}/categories/${archiveId}?replacementCategoryId=${encodeURIComponent(replacementId)}`
                    : `/equipment/${householdId}/categories/${archiveId}`;
            ...
            <button
              type="button"
              className="rounded-md border border-border px-4 py-2 text-sm self-end"
              onClick={() => {
                setArchiveId(null);
                setReplacementId("");
              }}
